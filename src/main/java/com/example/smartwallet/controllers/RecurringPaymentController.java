...
public class RecurringPaymentController {
    ...
    @FXML private CheckBox weeklyCheck;
    ...
    public void initialize() {
        ...
        if (monthlyCheck != null) {
            monthlyCheck.selectedProperty().addListener((obs, oldVal, newVal) -> {
                updateFilter();
                updatePagination();
            });
        }
        if (yearlyCheck != null) {
            yearlyCheck.selectedProperty().addListener((obs, oldVal, newVal) -> {
                updateFilter();
                updatePagination();
            });
        }
        if (weeklyCheck != null) {
            weeklyCheck.selectedProperty().addListener((obs, oldVal, newVal) -> {
                updateFilter();
                updatePagination();
            });
        }
        ...
        internalListView.setCellFactory(lv -> new ListCell<>() {
            ...
            protected void updateItem(RecurringPayment item, boolean empty) {
                ...
                if (empty || item == null) {
                    ...
                    nextDate.setStyle("-fx-text-fill: #6b7280; -fx-font-size: 11px;");
                    ...
                    amount.setStyle("-fx-text-fill: #111827; -fx-font-weight: bold; -fx-font-size: 14px;");
    ...
    private void updateFilter() {
        ...
        boolean showMonthly = monthlyCheck != null && monthlyCheck.isSelected();
        boolean showYearly = yearlyCheck != null && yearlyCheck.isSelected();
        boolean showWeekly = weeklyCheck != null && weeklyCheck.isSelected();

        filteredData.setPredicate(p -> {
            boolean nameMatch = p.getName().toLowerCase().contains(searchText);
            
            boolean freqMatch = false;
            RecurringPayment.Frequency f = p.getFrequency();
            if (showMonthly && f == RecurringPayment.Frequency.MONTHLY) freqMatch = true;
            if (showYearly && f == RecurringPayment.Frequency.YEARLY) freqMatch = true;
            if (showWeekly && f == RecurringPayment.Frequency.WEEKLY) freqMatch = true;
    ...
    private void refreshList() {
        try {
            ...
            List<RecurringPayment> list = service.recupererParUser(userId);
    ...
    private void payNow() {
        ...
        if ("Cycles".equals(unit)) {
            ...
            switch (selected.getFrequency()) {
                ...
                case WEEKLY: totalAmount = (selected.getAmount() / 4.0) * val; break;
            }
            ...
            switch (selected.getFrequency()) {
                ...
                case WEEKLY: totalAmount = selected.getAmount() * 52.0 * val; break;
            }
        }
            ...
            new Thread(() -> {
                try {
                    ...
                    String emailToUse = (sessionUser != null) ? sessionUser.getEmail() : com.example.smartwallet.utils.UserSession.getInstance().getUserEmail();
                    EmailService.sendPaymentSuccessEmail(emailToUse, intent.getId(), finalTotalAmount, "usd");

                    selected.setNextPaymentDate(finalNewDate);
                    service.modifier(selected);

                    Platform.runLater(() -> {
                        paymentStatus.setText("Payment successful: " + String.format("%.2f", finalTotalAmount) + " USD for " + finalVal + " " + finalUnit + "!");
    ...
    private void exportToPdf() {
        ...
        if (file != null) {
            try {
                ...
                document.open();
                document.add(new Paragraph("Recurring Payments"));
                ...
                for (RecurringPayment rp : filteredData) {
                    table.addCell(rp.getName());
                    table.addCell(String.format("%.2f", rp.getAmount()));
                    table.addCell(rp.getFrequency().name());
                    table.addCell(rp.getNextPaymentDate() != null ? rp.getNextPaymentDate().toString() : "N/A");
                }
    ...
    private void exportToExcel() {
        ...
        if (file != null) {
            try (Workbook workbook = new XSSFWorkbook()) {
                Sheet sheet = workbook.createSheet("Recurring Payments");

                Row headerRow = sheet.createRow(0);
                headerRow.createCell(0).setCellValue("Name");
                headerRow.createCell(1).setCellValue("Amount");
                headerRow.createCell(2).setCellValue("Frequency");
                headerRow.createCell(3).setCellValue("Next Date");

                int rowNum = 1;
                for (RecurringPayment rp : filteredData) {
                    Row row = sheet.createRow(rowNum++);
                    row.createCell(0).setCellValue(rp.getName());
                    row.createCell(1).setCellValue(rp.getAmount());
                    row.createCell(2).setCellValue(rp.getFrequency().name());
                    row.createCell(3).setCellValue(rp.getNextPaymentDate() != null ? rp.getNextPaymentDate().toString() : "N/A");
                }
