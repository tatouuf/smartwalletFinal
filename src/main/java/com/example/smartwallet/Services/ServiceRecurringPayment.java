...
public class ServiceRecurringPayment implements IService<RecurringPayment> {
    ...
    public void ajouter(RecurringPayment r) throws SQLException {
        ...
        String req = "INSERT INTO recurring_payments (user_id, profile_id, name, amount, frequency, next_payment_date, is_active) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?)";

        PreparedStatement ps = cnx.prepareStatement(req);
        ps.setInt(1, r.getUserId());
        ps.setInt(2, r.getProfileId());
        ps.setString(3, r.getName());
        ps.setDouble(4, r.getAmount());
        ps.setString(5, r.getFrequency().name());
        ps.setDate(6, Date.valueOf(r.getNextPaymentDate()));
