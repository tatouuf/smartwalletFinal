...
public class AIService {
    ...
    private AIService() {
        httpClient = HttpClient.newBuilder()
                ...
                .build();
        logger.info("AI Service initialized (ANTHROPIC MODE)");
    }
    ...
    // ================= REPLY SUGGESTIONS (ANTHROPIC API) =================
    public String generateReplySuggestion(String category, String userMessage) {

        if (ANTHROPIC_API_KEY == null || ANTHROPIC_API_KEY.isBlank()) {
            logger.warning("No Anthropic API key configured — using fallback reply.");
            return getFallbackReply(category);
        }

        try {
            String prompt = String.format(
                    "You are a customer support agent for SmartWallet, a digital wallet app. " +
                            "A user submitted the following complaint (category: %s):\n\n\"%s\"\n\n" +
                            "Write a professional, empathetic, and concise support reply in 2-3 sentences. " +
                            "Do not use placeholders like [Name]. Sign off as 'SmartWallet Support Team'.",
                    category.replace("_", " "), userMessage
            );
            ...
            if (response.statusCode() == 200) {
                ...
                logger.warning("Anthropic API returned status " + response.statusCode()
                        + " — using fallback.");
                return getFallbackReply(category);
            }

        } catch (Exception e) {
            logger.log(Level.WARNING, "Anthropic API call failed — using fallback", e);
            return getFallbackReply(category);
        }
    }
