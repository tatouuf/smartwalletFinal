...
public class ServiceUser extends BaseService implements IService<User> {
    ...
    public void ajouter(User user) throws SQLException {
        ...
        String req = "INSERT INTO users (nom, prenom, telephone, email, password, role, status, " +
                "date_creation, date_update, is_actif, nationalite, pays_residence) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            ...
            ps.setString(11, (user.getNationalite()   != null && !user.getNationalite().isEmpty())   ? user.getNationalite()   : null);
            ps.setString(12, (user.getPaysResidence() != null && !user.getPaysResidence().isEmpty()) ? user.getPaysResidence() : null);
            ps.executeUpdate();
        }
    ...
    // ================== UPDATE PROFILE ==================
    // Updates only nom, prenom, telephone, nationalite, pays_residence.
    // Does NOT touch password, role, status, is_actif.
    public void updateProfile(int userId, String nom, String prenom, String telephone,
                              String nationalite, String paysResidence) throws SQLException {
        verifierConnexion();

        String req = "UPDATE users SET nom=?, prenom=?, telephone=?, nationalite=?, " +
                "pays_residence=?, date_update=NOW() WHERE id=?";

        try (PreparedStatement ps = cnx.prepareStatement(req)) {
            ps.setString(1, nom);
            ps.setString(2, prenom);
            ps.setString(3, telephone);
            ps.setString(4, (nationalite != null && !nationalite.isEmpty()) ? nationalite : null);
            ps.setString(5, (paysResidence != null && !paysResidence.isEmpty()) ? paysResidence : null);
            ps.setInt(6, userId);
            int updated = ps.executeUpdate();
            if (updated == 0) throw new SQLException("No user found with ID: " + userId);
        }
        System.out.println("✅ Profile updated for user ID " + userId);
    }

    // ================== UPDATE PROFILE IMAGE ==================
    // Stores the image filename in the DB (file is copied by ProfileController).
    public void updateProfileImage(int userId, String imageFileName) throws SQLException {
        verifierConnexion();

        String req = "UPDATE users SET image_name=?, image_updated_at=NOW() WHERE id=?";
        try (PreparedStatement ps = cnx.prepareStatement(req)) {
    ...
    private User mapUser(ResultSet rs) throws SQLException {
        return new User(
                ...
                safeGetString(rs, "nationalite"),
                safeGetString(rs, "pays_residence"),
                safeGetString(rs, "image_name")
        );
    }
    ...
    private User mapSimpleUser(ResultSet rs) throws SQLException {
        ...
        user.setNationalite(safeGetString(rs, "nationalite"));
        user.setPaysResidence(safeGetString(rs, "pays_residence"));
        user.setImageName(safeGetString(rs, "image_name"));
        return user;
    }

    // ================== SAFE COLUMN READ ==================
    // Prevents crashes if migration hasn't been run yet.
