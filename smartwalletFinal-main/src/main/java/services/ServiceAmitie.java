...
public class ServiceAmitie {
    ...
    public void addFriend(int friendId) throws SQLException {
        ...
        String sql = "INSERT INTO amitie(userId, friendId, statut, dateCreation) VALUES (?, ?, ?, NOW())";
    ...
    public void acceptFriend(int requesterId) throws SQLException {
        int currentUserId = Session.getCurrentUser().getId();
        String sql = "UPDATE amitie SET statut = ? WHERE userId = ? AND friendId = ? AND statut = ?";
    ...
    public void deleteFriend(int requesterId) throws SQLException {
        int currentUserId = Session.getCurrentUser().getId();
        String sql = "DELETE FROM amitie WHERE userId = ? AND friendId = ? AND statut = ?";
    ...
    public void removeFriend(int friendId) throws SQLException {
        ...
        String sql = "DELETE FROM amitie WHERE " +
                "((userId = ? AND friendId = ?) OR (userId = ? AND friendId = ?)) AND statut = ?";
    ...
    public List<Amitie> getPendingRequests() throws SQLException {
        ...
        int currentUserId = Session.getCurrentUser().getId();
        String sql = "SELECT * FROM amitie WHERE friendId = ? AND statut = ? ORDER BY dateCreation DESC";
    ...
    // ================= COUNT ACCEPTED FRIENDS FOR A GIVEN USER (admin use) =================
    public long countAcceptedFriendsForUser(int userId) throws SQLException {
        String sql = "SELECT COUNT(*) FROM amitie WHERE (userId = ? OR friendId = ?) AND statut = ?";
        try (PreparedStatement ps = cnx.prepareStatement(sql)) {
            ps.setInt(1, userId);
            ps.setInt(2, userId);
    ...
    public List<Amitie> getAcceptedFriends() throws SQLException {
        List<Amitie> list = new ArrayList<>();
        int currentUserId = Session.getCurrentUser().getId();
        String sql = "SELECT * FROM amitie WHERE (userId = ? OR friendId = ?) AND statut = ?";
    ...
    public List<Amitie> getBlockedUsers() throws SQLException {
        List<Amitie> list = new ArrayList<>();
        int currentUserId = Session.getCurrentUser().getId();
        String sql = "SELECT * FROM amitie WHERE userId = ? AND statut = ?";
    ...
    public List<Amitie> getSentRequests() throws SQLException {
        List<Amitie> list = new ArrayList<>();
        int currentUserId = Session.getCurrentUser().getId();
        String sql = "SELECT * FROM amitie WHERE userId = ? AND statut = ? ORDER BY dateCreation DESC";
    ...
    public User getUserById(int id) throws SQLException {
        return userService.recupererParId(id);
    }
    ...
    public List<User> searchUsers(String keyword) throws SQLException {
        ...
        int currentUserId = Session.getCurrentUser().getId();
        // Search by email, nom OR prenom so typing an email address always works
        String sql = "SELECT * FROM users " +
                "WHERE (email LIKE ? OR nom LIKE ? OR prenom LIKE ?) " +
                "AND id != ? AND status = 'APPROVED' AND is_actif = 1";
    ...
    private Amitie mapAmitie(ResultSet rs) throws SQLException {
        ...
        a.setUser_id(rs.getInt("userId"));
        a.setFriend_id(rs.getInt("friendId"));
