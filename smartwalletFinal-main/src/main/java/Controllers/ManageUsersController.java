...
public class ManageUsersController {
    ...
    @FXML private Label lblUserId;       // FIX: was declared in FXML but missing here
    ...
    private void loadUserDetails(User user) {
        ...
        if (lblUserId != null) lblUserId.setText(String.valueOf(user.getId()));
    ...
    private void loadUserStatistics(int userId) {
        try {
            // Count friends directly via SQL for the given userId (not the session user)
            long friendsCount = amitieService.countAcceptedFriendsForUser(userId);
