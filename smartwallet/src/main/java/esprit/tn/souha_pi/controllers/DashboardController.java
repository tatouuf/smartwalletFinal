...
public class DashboardController {
    ...
    @FXML
    private Label noWalletLabel;
    @FXML
    private Button createWalletButton;

    // nouveau conteneur pour afficher un aperçu des dépenses
    @FXML
    ...
    public void refreshDashboard() {
        try {
            ...
            // mettre à jour l'aperçu des dépenses
            showRecentExpenses();
    ...
    private void goExpenses() {
        // redirige vers l'historique en filtrant les débits
        WalletLayoutController.instance.goHistory();
        // le contrôleur HistoryController pourra lui-même appliquer le filtre approprié
    }

    /**
     * Remplit le VBox expensesContainer avec les dernières dépenses de
     * l'utilisateur.
     */
    private void showRecentExpenses() {
        ...
        expensesContainer.getChildren().clear();
        try {
            var txService = new esprit.tn.souha_pi.services.TransactionService();
            var debits = txService.getDebitTransactions(currentUser.getId());
            int max = Math.min(debits.size(), 5); // ne montrer que les 5 plus récentes
            for (int i = 0; i < max; i++) {
                var t = debits.get(i);
                HBox row = new HBox(6);
                FontIcon icon = new FontIcon("fas-shopping-cart");
                icon.setIconSize(16);
                Label lbl = new Label(t.getTarget() + " – " + String.format("%.2f TND", Math.abs(t.getAmount())));
                row.getChildren().addAll(icon, lbl);
                expensesContainer.getChildren().add(row);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
