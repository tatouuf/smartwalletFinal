...
public class HistoryController implements Initializable {
    ...
    @FXML
    private TableColumn<Transaction, Number> colAmount;
    @FXML
    private TableColumn<Transaction, String> colWith;
    @FXML
    ...
    private boolean filterByType(Transaction t) {
        ...
        if ("Envois".equals(selected)) {
            return t.getType().contains("SEND");
        }
        if ("Réceptions".equals(selected)) {
            return t.getType().contains("RECEIVE") || t.getAmount() > 0;
        }
        return true;
    }
    ...
    /**
     * Public helper for callers that wish to show only debit transactions.
     * Should be invoked after the controller has been initialized.
     */
    public void showDebits() {
        if (typeFilter != null) {
            typeFilter.setValue("Débits");
            appliquerFiltres();
        }
    }
